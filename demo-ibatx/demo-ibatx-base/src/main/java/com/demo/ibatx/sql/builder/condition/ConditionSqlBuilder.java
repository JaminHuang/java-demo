...
import com.demo.ibatx.core.entity.ParamCondition;
import com.demo.ibatx.core.entity.ParamConstant;
import com.demo.ibatx.core.entity.ParamRelEnum;
...
public class ConditionSqlBuilder extends WhereSqlBuilder {
    ...
    private void parserCriteria(StringBuilder conditionSql, Condition.Criteria criteria, Map<String, Object> params) {
        ...
        for (int i = 0; i < conditions.size(); i++) {
            ...
            if (i != 0) {
                // 参数链接符号 AND 还是 OR
                if (Objects.nonNull(param.getParamRel())) {
                    conditionSql.append(param.getParamRel().getValue());
                } else {
                    conditionSql.append(ParamRelEnum.AND.getValue());
                }
            }
            ...
            if (param.getParamSymbol().getParamType() == ParamConstant.PARAM_SINGLE) {
                ...
                params.putIfAbsent(param.getParamName(), param.getValue());
